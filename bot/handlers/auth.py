    ...
    def handle_password(call: types.CallbackQuery):
        ...
        if action == "done":
            ...
            try:
                ...
                user_password_input[chat_id] = ""  # сбрасываем старый ввод
                # ❗ Используем send_message, чтобы клавиатура отобразилась снова
                tbot.send_message(chat_id, "Неверный пароль. Повторите ввод:",
                                  reply_markup=password_keyboard())
        else:
            # Добавляем символ к паролю
            user_password_input[chat_id] += action

            # 👇 Временный вывод текущего ввода
            # tbot.send_message(chat_id, f"Текущий ввод: {user_password_input[chat_id]}")
            # logging.info(f"[DEBUG] Пароль от {chat_id}: {user_password_input[chat_id]}")
